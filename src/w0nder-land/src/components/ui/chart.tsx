...
const ChartStyle = ({ id, config: chartConfig }: { id: string; config: ChartConfig }) => {
  const colorConfig = Object.entries(chartConfig).filter(([, config]) => config.theme || config.color);
...
function ChartTooltipContent({
  ...
  const tooltipLabel = React.useMemo(() => {
    ...
    if (labelFormatter) {
      return <div className={cn('font-heading', labelClassName)}>{labelFormatter(value, payload)}</div>;
    }
    ...
    <div className={cn('flex items-center justify-center gap-4', verticalAlign === 'top' ? 'pb-3' : 'pt-3', className)}>
      {payload.map(item => {
        ...
        return (
          <div
            key={item.value}
            className={cn('[&>svg]:text-foreground flex items-center gap-1.5 [&>svg]:h-3 [&>svg]:w-3')}>
