...
function SidebarGroup({ className, ...props }: React.ComponentProps<'div'>) {
  return (
    <div
      ...
      data-sidebar="group"
      className={cn('relative flex w-full min-w-0 flex-col p-2 border-b-2 border-b-border last:border-b-0', className)}
      {...props}
    />
  );
}
...
function SidebarMenuButton({
  ...
  const Comp = asChild ? Slot : 'button';
  const tooltip = typeof tooltipProp === 'string' ? { content: tooltipProp } : tooltipProp;
    ...
    <Tooltip>
      <TooltipTrigger asChild>
        <Comp
          ...
          className={cn(sidebarMenuButtonVariants({ size }), isActive && 'bg-black text-white', className)}
          {...props}
        />
      </TooltipTrigger>
...
function SidebarMenuAction({
  ...
  return (
    <Comp
      ...
      className={cn(
        'text-muted-foreground hover:text-foreground flex size-6 items-center justify-center rounded-base transition-colors',
