...
const IconLeft = ({ className: iconClassName, ...iconProps }: React.ComponentProps<typeof ChevronLeft>) => (
  <ChevronLeft className={cn('size-4', iconClassName)} {...iconProps} />
);

const IconRight = ({ className: iconClassName, ...iconProps }: React.ComponentProps<typeof ChevronRight>) => (
...
function Calendar({ className, classNames, showOutsideDays = true, ...props }: CalendarProps) {
  return (
    <DayPicker
      ...
      className={cn('rounded-base! border-2 border-border bg-main p-3 font-heading shadow-shadow', className)}
      classNames={{
        months: 'flex flex-col sm:flex-row gap-2',
        month: 'flex flex-col gap-4',
        caption: 'flex justify-center pt-1 relative items-center w-full text-main-foreground',
        caption_label: 'text-sm font-heading',
        nav: 'gap-1 flex items-center',
        nav_button: cn(buttonVariants({ variant: 'noShadow' }), 'size-7 bg-transparent p-0'),
        nav_button_previous: 'absolute left-1',
        nav_button_next: 'absolute right-1',
        table: 'w-full border-collapse space-y-1',
        head_row: 'flex',
        head_cell: 'text-main-foreground rounded-base w-9 font-base text-[0.8rem]',
        ...
        day: cn(buttonVariants({ variant: 'noShadow' }), 'size-9 p-0 font-base aria-selected:opacity-100'),
